...
select ma_khach_hang,ho_ten,ngay_sinh,timestampdiff(year,ngay_sinh,now()) as tuoi,so_cmnd,so_dien_thoai,email,dia_chi from khach_hang
...
select kh.ma_khach_hang,kh.ho_ten,count( hop_dong.ma_hop_dong is not null) as so_lan_dat_hang
from khach_hang kh,hop_dong,loai_khach
where kh.ma_loai_khach = loai_khach.ma_loai_khach and kh.ma_khach_hang=hop_dong.ma_khach_hang 
	and loai_khach.ten_loai_khach like 'Diamond'
...
-- ngay_ket_thuc, tong_tien (tổng tiền = Chi Phí Thuê + Số Lượng * Giá,Số Lượng và Giá là từ dich_vu_di_kem,hop_dong_chi_tiet) 
-- cho tất cả các khách hàng đã từng đặt phòng. (những khách hàng nào chưa từng đặt phòng cũng phải hiển thị ra).

select kh.ma_khach_hang, kh.ho_ten, lk.ten_loai_khach, hd.ma_hop_dong, dv.ten_dich_vu, hd.ngay_lam_hop_dong,hd.ngay_ket_thuc,
 (dv.chi_phi_thue is not null) + (dvdk.gia is not null)*(hdct.so_luong is not null) as tong_tien
from  khach_hang kh, loai_khach lk,hop_dong hd,hop_dong_chi_tiet hdct,dich_vu_di_kem dvdk, dich_vu dv
where kh.ma_loai_khach=lk.ma_loai_khach and kh.ma_khach_hang=hd.ma_khach_hang
	and hd.ma_dich_vu=dv.ma_dich_vu and hd.ma_hop_dong=hdct.ma_hop_dong and hdct.ma_dich_vu_di_kem=dvdk.ma_dich_vu_di_kem
...
from dich_vu dv, hop_dong hd, loai_dich_vu ldv
where dv.ma_loai_dich_vu = ldv.ma_loai_dich_vu and dv.ma_dich_vu=hd.ma_dich_vu 
...
select month(hd.ngay_lam_hop_dong), count(kh.ma_khach_hang) as so_luong_khach_hang from khach_hang kh, hop_dong hd
...
-- Câu 11: Hiển thị thông tin các dịch vụ đi kèm đã được sử dụng bởi 
...
select hd.ma_hop_dong,nv.ho_ten,kh.ho_ten,kh.so_dien_thoai,dv.ten_dich_vu, ifnull(sum(hdct.so_luong),0) as so_luong_dich_vu_di_kem ,
hd.tien_dat_coc
from hop_dong hd
right join nhan_vien nv on hd.ma_nhan_vien=nv.ma_nhan_vien
right join khach_hang kh on hd.ma_khach_hang=kh.ma_khach_hang
right join dich_vu dv on hd.ma_dich_vu=dv.ma_dich_vu
...
select ma_dich_vu_di_kem, ten_dich_vu_di_kem,sum(so_luong) tong_so_luong_dvdk
from dich_vu_di_kem
JOIN hop_dong_chi_tiet USING(ma_dich_vu_di_kem)
group by ma_dich_vu_di_kem
having tong_so_luong_dvdk >= all(
        select sum(so_luong) tong_so_luong_dvdk
        from hop_dong_chi_tiet
        group by ma_dich_vu_di_kem);
-- Câu 14: Hiển thị thông tin tất cả các Dịch vụ đi kèm chỉ mới được sử dụng một lần duy nhất.
-- Thông tin hiển thị bao gồm ma_hop_dong, ten_loai_dich_vu, ten_dich_vu_di_kem, so_lan_su_dung (được tính
-- dựa trên việc count các ma_dich_vu_di_kem).   

select dvdk.ma_dich_vu_di_kem,dvdk.ten_dich_vu_di_kem,dvdk.gia,
 count(hdct.ma_dich_vu_di_kem) as tong 
...
-- Câu 15:	Hiển thi thông tin của tất cả nhân viên bao gồm ma_nhan_vien, ho_ten, ten_trinh_do, ten_bo_phan, so_dien_thoai, 
